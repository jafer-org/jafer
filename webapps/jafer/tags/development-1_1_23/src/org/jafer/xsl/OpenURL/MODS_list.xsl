...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0"
  ...
  <xsl:template match="mods:mods"><!-- linking uses DocID -->
   <li>
     <a><xsl:attribute name="href">OpenURL?sid=jafer:servlet&amp;pid=<xsl:value-of select="mods:recordInfo/mods:recordIdentifier"/></xsl:attribute>
     <xsl:value-of select="mods:name/text()"/>
     <xsl:text> - </xsl:text>
     <xsl:value-of select="mods:title/text()"/></a>
   </li>
  </xsl:template>
