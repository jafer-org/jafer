 ...
 * <p>A default search profile is used if one has been set using an appropriate constructor.</p>
 * <p>Some getNode() methods allow the default search profile to be altered, or ignored for that query.</p>
 *
 * @see: <code>org.jafer.conf.Bib1Attributes.xml</code> for valid attribute values.
 * @see Schema of XML structure available at <a href="http://www.jafer.org">www.jafer.org</a>.
 */
 ...
 * <p>The profile parameter is interpreted in the following order:</p>
 * <p>[1] Use, [2] Relation, [3] Position, [4] Structure, [5] Truncation, [6] Completeness</p>
 * <p>A value of zero in the array will be ignored, and can be used as a spacer.</p>
 * <p>(Default search profile can be overidden when using certain getNode() methods.)</p>
 *
...
/**
 * Constructs a new QueryBuilder with a default Bib1 search profile which will be used in creating queries.
 * <p>The profile parameter consists of a dot delimited String (eg: 4.3.3.101.100.1), with values interpreted in the following order:</p>
 * <p>Use, Relation, Position, Structure, Truncation, Completeness.</p>
 * <p>A value of zero will be ignored, and can be used as a spacer.</p>
 * <p>(Default search profile can be overidden when using certain getNode() methods.)</p>
 *
 * @param defaultSearchProfile  dot delimited String with at least 1, and a maximum of 6 items.
 */
 ...
 * <p>Attribute supplied must be the name of a Use attribute (eg: "Title"),
 * or a String representation of its corresponding numeric value (eg: "4").</p>
 * <p>Note: If an attribute name is supplied, it is checked against lookup table via <code>org.jafer.conf.Config.getAttributeValue()</code>,
 * but not if a corresponding numeric value is used, in which case any positive value within the int range is accepted.</p>
 *
 * @param  useAttribute  represents the Use attribute to be used in the query, e.g. "title".
 * @param  term          the term to be used in the query.
 * @return               a query node.
 * @throws org.jafer.zclient.QueryException if <code>useAttribute</code> parameter is not valid, and <code>term</code> is empty..
 */
...
/**
 * Creates a basic query node incorporating the Use attribute and term supplied.
 * <p>Attribute value is not checked, any positive int value is accepted.</p>
 *
 * @param  useAttribute  represents the Use attribute to be used in the query.
 * @param  term          the term to be used in the query.
 * @return               a query node.
 * @throws org.jafer.query.QueryException if parameters are not valid or empty.
 */
 ...
 * <p>The int[] parameter values are used in order for:</p>
 * <p>{Use, Relation, Position, Structure, Truncation, Completeness}</p>
 * <p>e.g. { 4, 3, 3, 2, 100, 1 }</p>
 * <p>A smaller sized array can be used, in which case the values given will
 *  be interpreted in order, following the pattern above.</p>
 * <p>A parameter value of 0 will cause the corresponding attribute type to be
 * excluded from the query.</p>
 * <p>(If a default search profile has been set, it will be overridden for this query:
 * any values supplied in the parameter will replace corresponding default values.)</p>
 *
 * @param  attributes  represents the attribute values to be used in the query.
 * @param  term        the term to be used in the query.
 * @return             a query node.
 * @throws org.jafer.zclient.QueryException if <code>attributes</code> array size
 * is smaller than 1, or larger than 6.
 */
  public Node getNode(int[] attributes, String term) throws QueryException {
   /* copies defaultAttributes as initial array, with all values shifted up by
    one position (as array position [0] is ignored in buildConstraintModelNode()).
    Then copies any values that have been set in attributes parameter.  */
 ...
 * <p>The int[][] parameter holds the attribute types and corresponding values.</p>
 * <p>e.g. {{1,1003}, {2,3}, {4,2}} would set:</p>
 * <p>Use = 1003, Relation = 3 and Structure = 2.</p>
 * <p>A parameter value of 0 will result in the corresponding attribute type to be
 * excluded from the query.</p>
 * <p>(If a default search profile has been set, it will be altered for this query:
 * any values given in the parameter will replace corresponding default values.)</p>
 *
 * @param  attTypesValues  represents the attribute types and their corresponding
 * values to be used in the query.
 * @param  term        the term to be used in the query.
 * @return             a query node.
 * @throws org.jafer.zclient.QueryException if <code>attTypesValues</code> array size is smaller than 1, or larger than 6.
 * Also if the first attribute type is missing, or not a positive value.
 */
  public Node getNode(int[][] attTypesValues, String term) throws QueryException {

  /* copies defaultAttributes as initial array, with all values shifted up by
  one position (as array position [0] is ignored in buildConstraintModelNode()).
  Then copies any values that have been set in attTypesValues parameter.
  */

    if(attTypesValues.length < 1 || attTypesValues.length > 6)
      throw new QueryException("Invalid attribute array length, should be between 1 and 6");

    if(attTypesValues[0].length < 2 || attTypesValues[0][1] < 1)
  ...
  /**
   * Creates a basic query node incorporating the term and attributes supplied.
   * <p>The String[] parameter holds the term to be used in position [0], and
   * the remaining attributes thereafter, eg:</p>
   * <p>{"Shakespeare", "1003", "3", "3", "2", "100", "1"}</p>
   *
   * <p>Shorter arrays can be used, and values of "0" will cause that attribute to be omitted from the query.</p>
   * <p>(If a default search profile has been set, it will be ignored for this query)</p>
   *
   * @param  termAndAttributes  the term (position [0]) and attributes to be used in the query.
   * @return             a query node.
 ...
 * @param  term        the term to be used in the query.
 * @return             a query node.
 * @throws org.jafer.zclient.QueryException if there is an error in the attribute lookup, or the term is empty.
 */
  ...
  private Node buildConstraintModelNode(int[] attributes, String term) throws QueryException {
    ...
    constraint = constraintModel.appendChild(
                                document.createElement("constraint"));
    n = constraintModel.appendChild(document.createElement("model"));
    n.appendChild(document.createTextNode(term));
/*
 *  loop starts at 1, as attributes[0] is undefined.
 *  only attributes that are not set to 0 are used in the query being built.
 *  a check is made to ensure that a Use attribute has been set.
 */
...
/**
 * Produces an AND node from a pair of attributes,
 * or a tree of AND nodes if more than 2 attributes are given.
 * <p>Vector parameters: Minimum size is 2, and must contain only String objects.</p>
 * <p>Attributes supplied must be the name of a Use attribute (eg: "Title"),
 * or a String representation of its corresponding numeric value (eg: "4").</p>
 * <p>Note: If an attribute name is supplied, it is checked against lookup table via <code>org.jafer.conf.Config.getAttributeValue()</code>,
 * but not if a corresponding numeric value is used, in which case any positive value within the int range is accepted.</p>
 *
 * @param  useAttributes the attribute values to be used in building the query.
 * @param  terms         the corresponding terms to be used in building the query.
 * @return               an AND node or tree.
 * @throws org.jafer.query.QueryException if a problem is encountered with the input Vectors.
 * @see: <code>org.jafer.util.Bib1Attributes.java</code>
 * for possible <code>useAttribute</code> values.
 */
 ...
 // to build a balanced tree from useAttributes with even number of elements:
  for (int i=1; i<useAttributes.size(); i+=2) {
    ...
    else currentNode = and(left, right);
    }
...
/**
 * Produces an OR node from a pair of attributes,
 * or a tree of OR nodes if more than 2 attributes are given.
 * <p>Vector parameters: minimum size is 2, and must contain only String objects.</p>
 * <p>Attributes supplied must be the name of a Use attribute (eg: "Title"),
 * or a String representation of its corresponding numeric value (eg: "4").</p>
 * <p>Note: If an attribute name is supplied, it is checked against lookup table via <code>org.jafer.conf.Config.getAttributeValue()</code>,
 ...
 */
 ...
 // to build a balanced tree from useAttributes with even number of elements:
  for (int i=1; i<useAttributes.size(); i+=2) {
    ...
    else currentNode = or(left, right);
    }
 // if useAttributes has odd number of elements, handle last element:
    if (useAttributes.size()%2 == 1) {
      int j = useAttributes.size() - 1;
 ...
 * i.e. produces a negated version of the supplied node.
 *
 ...
 */
  ...
  public Node getNode(String queryExp) throws QueryException {
    ...
    } else throw new QueryException("Null or empty query expression");
  }

/**
 * Method returns a representation of an XML query as a 2 dimensional String[].
 * <p>Operands follow operators.</p>
 * <p>An operator is a String[] containing 1 String, eg: {"and"}</p>
 * <p>An operand is a String[] containing a term, followed by up to 6 attributes. eg:</p>
 * <p>{"Shakespeare", "1003", "3", "3", "2", "100", "1"}</p>
 *
 * @param  queryNode the XML query node.
 * @return           A String[][] representation of a query.
 * @throws org.jafer.query.QueryException if a problem is found in the query structure.
 */
  ...
  private String[] getConstraintModelData(Node cM) throws QueryException {
    ...
    try {
//      NodeList attributes = Config.selectNodeList(cM, "./constraint/*");
